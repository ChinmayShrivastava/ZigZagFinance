...
contract VaultFactory {
    ...
    function createVault(uint _commission, string memory _vaultname, uint _exitfee, address _uniswapRouter, address _protocolToken) external returns (address) {
        Vault newVault = new Vault(msg.sender, _commission, _vaultname, _exitfee, _uniswapRouter, _protocolToken);
...
contract Vault {
    ...
    constructor(address _owner, uint _commission, string memory _vaultname, uint _exitfee, address _uniswapRouter, address _protocolToken) {
        owner = _owner;
    ...
    function deposit() external payable {
        ...
        redeemTime[msg.sender] = block.timestamp.add(1 days);
    ...
    function redeem() external {
        ...
        require(block.timestamp >= redeemTime[msg.sender]);
    ...
    function tradeOnUniswap(address[] memory path, uint amountIn, uint amountOutMin) external onlyOwner onlyWhitelistedPool {
        require(block.timestamp.sub(redeemTime[msg.sender]) >= 24 hours, "Only one trade per 24 hours is allowed");
        require(path.length >= 2, "Invalid trading path");
        
        uint[] memory amounts = uniswapRouter.swapExactTokensForTokens(amountIn, amountOutMin, path, address(this), block.timestamp.add(1 days));
        lastTradeTime = block.timestamp;
        
        uint commissionAmount = amounts[amounts.length - 1].mul(commission).div(100);
        uint tradeAmount = amounts[amounts.length - 1].sub(commissionAmount);
        protocolToken.transfer(owner, commissionAmount);
        balances[owner] = balances[owner].add(tradeAmount);
    }
    ...
    function voteForWhitelistedPool(address pool) external {
        ...
        vote.voteCount = vote.voteCount.add(protocolToken.balanceOf(msg.sender));
    ...
    function executeWhitelistedPoolVote(address pool) external onlyOwner {
        Vote storage vote = governanceVotes[pool];
        require(whitelistedPools[pool] == false, "Pool is already whitelisted");
        require(vote.voteCount >= protocolToken.totalSupply().div(2), "Insufficient votes");
